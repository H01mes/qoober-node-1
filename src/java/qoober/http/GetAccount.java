        ...
        if (includeCurrencies) {
            try (DbIterator<Account.AccountCurrency> accountCurrencies = account.getCurrencies(0, -1)) {
				...
				if(includeParamining){
					response.put("paramaningVirtualBalanceNQT", account.getVirtualParaminingBalanceNQT());
					response.put("paramaningStructBalanceNQT", account.getStructBalanceNQT(Qoober.getBlockchain().getHeight()) );
					response.put("paramaningStructCoefficient", Paramining.getAccountStructCoefficient(account.getId()) );
					response.put("paramaningStorageDays", Paramining.getAccountStorageDays(account.getId()) );
					response.put("paramaningStorageCoefficient", Paramining.getAccountStorageCoefficient(account.getId()) );
					response.put("paramaningBalanceCoefficient", String.valueOf(Paramining.getAccountBalanceCoefficient(account.getId())) );
					response.put("paramaningParatax", "0");
				}
